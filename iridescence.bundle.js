...
void main() {
  ...
  // Blend with white for a light pastel theme
  vec3 lightThemeBase = vec3(1.0, 0.96, 0.98); // Very light pinkish-white base
  col = mix(lightThemeBase, col, 0.15); // Only 15% iridescence, 85% white
  
  gl_FragColor = vec4(col, 1.0);
}
`;
  function initIridescence() {
    const ctn = document.getElementById("iridescence-container");
    if (!ctn) return;
    const color = [1, 0.75, 0.8];
    const speed = 1;
    const amplitude = 0.1;
    ...
    function resize() {
      const scale5 = 1;
      renderer.setSize(ctn.offsetWidth * scale5, ctn.offsetHeight * scale5);
      if (program) {
