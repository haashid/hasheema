...
`;
  function initLightfall() {
    ...
    const prepColors = (input) => {
      ...
      const count2 = base.length;
      const arr2 = [];
      for (let i = 0; i < MAX_COLORS; i++) arr2.push(hexToRGB(base[Math.min(i, base.length - 1)]));
      const avg2 = [0, 0, 0];
      ...
      avg2[1] /= count2;
      avg2[2] /= count2;
      return { arr: arr2, count: count2, avg: avg2 };
    };
    ...
    const colors = ["#A6C8FF", "#5227FF", "#FF9FFC", "#ed677e", "#F43F5E"];
    const backgroundColor = "#edb3d1";
    const speed = 1.1;
    const streakCount = 4;
    const streakWidth = 0.4;
    const streakLength = 1;
    const glow = 1;
    const density = 0.7;
    const twinkle = 1;
    const zoom = 3;
    const backgroundGlow = 1.5;
    const opacity = 1;
    ...
    const onPointerMove = (e) => {
      ...
      const scale5 = renderer.dpr || 1;
      const x = (e.clientX - rect.left) * scale5;
      const y = (rect.height - (e.clientY - rect.top)) * scale5;
      mouseTarget = [x, y];
      if (mouseDampening <= 0) {
